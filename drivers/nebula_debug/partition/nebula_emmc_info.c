...
struct nebula_emmc_partition {
	unsigned int dev_num;
	unsigned int partition_size;
	char partition_name[16];
};

static struct nebula_emmc_partition emmc_partitions[MSM_MAX_PARTITIONS];

static int nebula_emmc_partition_read(struct seq_file *m, void *v)
{
	struct nebula_emmc_partition *ptn = emmc_partitions;
	int i;
	unsigned int dev_num;
	unsigned int size;
...
static ssize_t nebula_emmc_partition_write(struct file *file, const char __user *buffer,
...
static int nebula_emmc_partition_open(struct inode *inode, struct file *file)
{
	return single_open(file, nebula_emmc_partition_read, NULL);
}

static const struct file_operations nebula_emmc_partition_fops = {
	.owner		= THIS_MODULE,
	.open		= nebula_emmc_partition_open,
	.write		= nebula_emmc_partition_write,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static ssize_t nebula_cancel_fsync_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *ppos)
{
	int val;
	char buf[64];
...
static int nebula_cancel_fsync_read(struct seq_file *m, void *v)
{
	return seq_printf(m, "%d", cancel_fsync);
}

static int nebula_cancel_fsync_open(struct inode *inode, struct file *file)
{
	return single_open(file, nebula_cancel_fsync_read, NULL);
}

static const struct file_operations nebula_cancel_fsync_fops = {
	.open           = nebula_cancel_fsync_open,
	.write          = nebula_cancel_fsync_write,
	.read           = seq_read,
	.llseek         = seq_lseek,
	.release        = single_release,
};
...
static int __init sysinfo_proc_init(void)
{
	...
	pr_info("%s: Init NEBULA system info proc interface.\r\n", __func__);

	entry = proc_create_data("cancel_fsync", 0644, NULL, &nebula_cancel_fsync_fops, NULL);
	if (entry == NULL)
	...
	entry = proc_create_data("emmc", 0644, NULL, &nebula_emmc_partition_fops, NULL);
	if (entry == NULL) {
...
MODULE_DESCRIPTION("NEBULA Partition Info Interface");
MODULE_VERSION("1.0");
