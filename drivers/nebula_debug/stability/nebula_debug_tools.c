...
#if defined(CONFIG_NEBULA_DEBUG_WATCHDOG)
...
static unsigned long long nebula_debug_watchdog_last_pet;

void nebula_debug_watchdog_check_pet(unsigned long long timestamp)
{
	if (!nebula_debug_watchdog_enabled() || !nebula_debug_watchdog_last_pet)
		return;

	if (timestamp - nebula_debug_watchdog_last_pet > (unsigned long long)NSEC_PER_THRESHOLD) {
		if (timestamp - last_pet_check > (unsigned long long)NSEC_PER_SEC) {
			last_pet_check = timestamp;
			pet_check_counter = timestamp - nebula_debug_watchdog_last_pet;
...
void nebula_debug_watchdog_update_last_pet(unsigned long long last_pet)
{
	nebula_debug_watchdog_last_pet = last_pet;
}

/* TODO: support this funciton with CONFIG_SPARSE_IRQ */
#if !defined(CONFIG_SPARSE_IRQ)
static unsigned int last_irqs[NR_IRQS];
void nebula_debug_watchdog_dump_irqs(unsigned int dump)
{
	int n;
	if (dump) {
		pr_debug("\nWatchdog dump irqs:\n");
...
#endif /* CONFIG_NEBULA_DEBUG_WATCHDOG */

/* n.b.:
 * 1. sched_clock is not irq safe
 * 2. 32 bit: overflows every 4,294,967,296 msecs
 */
unsigned long nebula_debug_get_sched_clock_ms(void)
{
