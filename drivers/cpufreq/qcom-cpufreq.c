...
static unsigned int upper_limit_freq_pro[NR_CPUS] = {2188800, 2188800, 2342400, 2342400};
static unsigned int upper_limit_freq[NR_CPUS] = {1593600, 1593600, 2150400, 2150400};
static unsigned int lower_limit_freq[NR_CPUS] = {307200, 307200, 307200, 307200};
#define CPU_MAX_OC_FREQ_PRO_BC	2342400 // OC NOT SET YET
#define CPU_MAX_OC_FREQ_PRO_LC	2188800 // OC NOT SET YET
#define CPU_MAX_OC_FREQ_BC	2150400 // OC NOT SET YET 2265600
#define CPU_MAX_OC_FREQ_LC	1593600 // OC NOT SET YET 1728000
...
void set_cpu_max_lock(unsigned int cpu, unsigned int freq)
{
	if (cpu >= 0 && cpu <= NR_CPUS) {
		/*
		 * LG G6 device has 2 cpu clusters,
		 * each 2 cores are linked with same freq and gov
		 * make sure they get set with same lock freq per cluster.
		 */
		if (freq == 0) {
			...
			if (cpu <= 1) {
				...
				if (freq < CPU_MIN_DEFAULT_FREQ ||
						freq > CPU_MAX_OC_FREQ_PRO_BC) {
					upper_limit_freq_pro[2] = CONFIG_CPU_FREQ_MAX_CLUSTER2;
					upper_limit_freq_pro[3] = CONFIG_CPU_FREQ_MAX_CLUSTER2;
				} else {
